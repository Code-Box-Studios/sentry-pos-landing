...
- Figtree across every UI surface (Euclid Circular A substituted)
...
### Font Family
**Figtree** (primary — substitutes the commercial Euclid Circular A; same geometric voice and weight scale): Fallbacks: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif.
...
- Maintain Figtree across every UI surface
...
**The mark:** a thermal receipt curling into the letter S — clean-cut edge at the top (fresh off the printer), a torn zigzag at the tail, printed lines riding the ribbon, and the green status dot at the fresh edge. The mark and the product are the same object.

**Files** (`brand/`): `sentry-mark.svg` (teal, for light surfaces) · `sentry-mark-reverse.svg` (white, for dark surfaces) · `sentry-mark-mono.svg` (single-color via `currentColor`, print lines omitted) · `sentry-favicon.svg` (teal tile app icon, thickened for tiny sizes) · `sentry-lockup.svg` (mark + Figtree 600 wordmark; convert the text to outlines before any external print use).

**Rules:** clear space equals the dot's diameter on all sides · minimum size 24px for the full mark, favicon variant below that · the dot is always green except in mono · the mark never appears on customer receipts (business branding only, pos-spec §6).
